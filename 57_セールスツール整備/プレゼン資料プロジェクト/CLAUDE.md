...
### 高優先
- ~~**見積書読み込み機能**~~ ✅ 実装済み（Session 24）
- **デマンドカット試算ロジック** — iPals 30分値CSVからPV導入前/後のピーク値を算出
  - PDF実物の構造: 上部にKPI(①導入前ピーク287kW ②導入後ピーク262kW ▲25kW) + 月額基本料金算出式
  - 2段の折れ線グラフ: 【PV導入前】使用電力量+発電量 / 【PV導入後】使用電力量（2週間分30分値）
  - 凡例: PV自家消費量, 充電量, 放電量, 使用電力量, ピークライン
  - 基本料金計算: 基本料金単価 × デマンドピーク値 × (力率割引1.85 - 1.00)
  - 必要データ: iPals CSV（1時間値）+ METPV-20日射量データベースの想定発電量
  - iPals CSV構造: 月,日,時(1-24),発電量(kWh),需要量(kWh),不足電力量,余剰電力量,自家消費電力量,自家消費率(%),消費率(%),モジュール出力(kWh)
  - サンプル: `プレゼン資料プロジェクト/input/自家消費発電量.csv`
  - 注意: 30分値ではなく1時間値。デマンド値は需要量列の最大値で近似可能
- **経済効果試算のEPC版** — EP4スライドの20年シミュレーション表をPDF実物と一致させる（従量料金はPPA単価ではなく電力量料金の削減額ベース）
...
## 次回セッション（Session 25）着手事項
1. **デマンドカット試算ロジック** — iPals CSV(1時間値)からピーク検出、基本料金削減額算出、PP9/EP5スライドに反映
2. **pp8a**: PPA時のselling_price使用修正（違約金テーブルの計算元）
3. **pp10**: PPA時の実質負担額計算修正
4. **EPC版スライドのPDF忠実化** — EP4(経済効果)の20年表をPDF実物に合わせる
