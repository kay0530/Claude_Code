...
**メーカー名の表記差異（見積書 vs SF機器マスタ）:**
- 見積書(F列): `DMEGC_Solar`, `Jinko_Solar`, `Huawei` 等（アンダースコア区切り）
- SF機器マスタ(Maker__c): `DMEGC Solar`, `JinkoSolar`, `Huawei` 等（スペース区切りやキャメルケース）
- → 照合時にアンダースコア除去・大文字小文字無視・スペース正規化が必要
