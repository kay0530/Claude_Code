...
### 見積書読み込み機能（新規要件）
**目的**: 見積書（PDF/Excel）をアップロードすると、パネル・PCS・蓄電池のメーカー・型式・数量・金額を自動抽出
- Tab 2の「📄 見積書から読み込み（準備中）」UIが既にプレースホルダーとして存在
- 抽出対象: パネル（メーカー・型式・W数・枚数）、PCS（メーカー・型式・kW・台数）、蓄電池（メーカー・型式・kWh・台数）、原価・販売価格
- 見積書フォーマットは当社の標準テンプレート or 協力会社からの見積書
- 実装方針: PDF→テキスト抽出→LLMまたはルールベースで構造化 or Excel見積書→openpyxlで直接パース
- SF機器マスタとの照合（型式名マッチング）で確度を上げる
